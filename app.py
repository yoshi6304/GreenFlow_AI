...
<style>
/* ── FONTS ── */
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:ital,opsz,wght@0,10..85,300;0,10..85,400;0,10..85,500;0,10..85,600;0,10..85,700;0,10..85,800&family=Familjen+Grotesk:wght@700;800&display=swap');
...
/* ── BASE TYPOGRAPHY ── */
html, body, [class*="css"], p, div, span, li, td, th, label {
...
[data-testid="metric-container"] {
    background: linear-gradient(145deg, rgba(10,28,60,0.95), rgba(15,40,80,0.85)) !important;
...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #050f1f 0%, #071428 60%, #050d1c 100%) !important;
...
.stCheckbox label { color: #a0c4d8 !important; font-size: 0.9rem !important; }
...
.stTabs [aria-selected="true"] {
    background: linear-gradient(135deg, rgba(14,165,233,0.2), rgba(16,185,129,0.12)) !important;
...
/* ── CUSTOM CARD ── */
.gf-card {
    background: linear-gradient(145deg, rgba(8,22,50,0.95), rgba(12,30,65,0.85));
...
.gf-hero-title {
    ...
    font-weight: 800;
    background: linear-gradient(135deg, #f0f9ff 0%, #38bdf8 45%, #34d399 100%);
...
.gf-alert-info {
    background: linear-gradient(135deg, rgba(56,189,248,0.07), rgba(14,165,233,0.03));
...
.stat-pill {
    ...
    align-items: center;
    padding: 16px 26px;
    background: linear-gradient(145deg, rgba(14,165,233,0.09), rgba(16,185,129,0.05));
    border: 1px solid rgba(56,189,248,0.2);
    border-radius: 18px;
    margin: 5px;
...
PLOT_BASE = dict(
    ...
    font=dict(family='Plus Jakarta Sans', color='#7bafc8', size=11),
...
def page_landing():
    ...
    # Feature cards
    st.markdown('<p style="font-family:Familjen Grotesk,sans-serif;font-size:1.9rem;font-weight:800;color:#00d4ff;text-align:center;margin-bottom:6px;">Why GreenFlow AI?</p>', unsafe_allow_html=True)
    ...
    # How it works
    st.markdown('<p style="font-family:Familjen Grotesk,sans-serif;font-size:1.9rem;font-weight:800;color:#00d4ff;margin-bottom:20px;">How It Works</p>', unsafe_allow_html=True)
    steps = [
    ...
    for num, title, desc in steps:
        ...
        with c1:
            st.markdown(f'<div style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#00d4ff,#00f5c4);display:flex;align-items:center;justify-content:center;font-family:Familjen Grotesk,sans-serif;font-weight:800;color:#0a1628;margin-top:3px;">{num}</div>', unsafe_allow_html=True)
        with c2:
...
def page_login():
    ...
    with col:
        ...
        <div style="text-align:center;margin-bottom:28px;">
            ...
            <p style="font-family:Familjen Grotesk,sans-serif;font-size:2rem;font-weight:800;
...
def page_dashboard():
    ...
    with st.sidebar:
        ...
        <div style="padding:6px 0 14px 0;">
            ...
            <p style="font-family:Familjen Grotesk,sans-serif;font-size:1.25rem;font-weight:800;
               background:linear-gradient(135deg,#00d4ff,#00f5c4);
               -webkit-background-clip:text;-webkit-text-fill-color:transparent;
        ...
        st.markdown(
            f'<div style="font-size:0.73rem;color:#8ab8d0;line-height:2.2;">'
            f'<div>🤖 Algorithm: Gradient Boosting</div>'
            f'<div>📈 R² Score: {_r2}</div>'
            f'<div>🎯 MAPE: {_mape}%</div>'
            f'<div>📉 MAE: {_mae:,.0f} kL</div>'
            f'<div>🏫 Campus: Bundoora</div>'
            f'<div>📅 Updated: {_date}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    with tb1:
        st.markdown(f'<p style="font-family:Familjen Grotesk,sans-serif;font-size:1.45rem;font-weight:800;color:#e8f4fd;margin:0;">{panel}</p>', unsafe_allow_html=True)
    with tb2:
        st.markdown('<div style="background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);border-radius:20px;padding:5px 14px;display:inline-flex;align-items:center;gap:8px;font-size:0.78rem;color:#34d399;font-weight:600;">🟢 Live System</div>', unsafe_allow_html=True)
    with tb3:
    ...
    for idx, (_, row) in enumerate(df_fc.iterrows()):
        with cols[idx % 3]:
            ...
            sessions_html = ''.join([
                ...
                f'<span style="font-family:Familjen Grotesk,sans-serif;font-size:1.1rem;font-weight:800;color:#00f5c4;">'
                f'{h:02d}:00</span>'
...
def panel_predict(model, df_feat):
    ...
    if submitted:
        ...
        st.markdown("---")
        # Pre-compute all values to avoid ternaries inside f-string (prevents broken HTML)
        tag_holiday  = "· Holiday"  if is_holiday  else ""
        tag_semester = "· Semester" if is_semester else ""
        tag_exam     = "· Exam"     if is_exam     else ""
        context_tags = f"{tag_holiday} {tag_semester} {tag_exam}".strip()
        trend_arrow  = "▲" if vs_avg_pct > 0 else "▼"
        trend_color  = "#fb923c" if vs_avg_pct > 10 else "#34d399" if vs_avg_pct < -10 else "#94a3b8"
        pred_fmt     = f"{pred:,.0f}"
        avg_fmt      = f"{avg_hist:,.0f}"
        pct_fmt      = f"{abs(vs_avg_pct):.1f}"

        result_html = f"""
        <div style="background:linear-gradient(135deg,rgba(14,165,233,0.10),rgba(52,211,153,0.06));
             border:1px solid rgba(56,189,248,0.28);border-radius:24px;
             padding:40px 32px;text-align:center;margin:16px 0;
             box-shadow:0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05);">
            <p style="color:#64a8c8;font-size:0.72rem;font-weight:700;text-transform:uppercase;
               letter-spacing:3px;margin-bottom:12px;">
               Predicted Water Demand &nbsp;·&nbsp; {dow} &nbsp;·&nbsp; {month}
               &nbsp; {context_tags}
            </p>
            <p style="font-family:Familjen Grotesk,sans-serif;font-size:5rem;font-weight:800;
               background:linear-gradient(135deg,#38bdf8,#34d399);
               -webkit-background-clip:text;-webkit-text-fill-color:transparent;
               background-clip:text;margin:0;line-height:1.05;letter-spacing:-2px;">
               {pred_fmt}
            </p>
            <p style="color:#64a8c8;font-size:1rem;margin:8px 0 20px 0;font-weight:500;">
               kilolitres (kL)
            </p>
            <p style="color:{trend_color};font-size:0.9rem;font-weight:600;
               background:rgba(255,255,255,0.04);border-radius:20px;
               padding:6px 20px;display:inline-block;border:1px solid rgba(255,255,255,0.08);">
               {trend_arrow} {pct_fmt}% vs historical average ({avg_fmt} kL)
            </p>
        </div>
        """
...
def panel_analytics(df_pred, df_feat, stats):
    ...
    <div style="background:linear-gradient(135deg,rgba(0,212,255,0.08),rgba(0,245,196,0.05));
        ...
        <p style="font-family:Familjen Grotesk,sans-serif;font-size:1.8rem;font-weight:800;
...
def panel_alerts(df_pred, df_fc):
    ...
    alerts = [
        ...
        ("warn", "🌊",
         ...
         "Weekend forecasts show different usage patterns on weekdays vs weekends. "
         "Adjust hostel and recreation facility allocations accordingly.",
